...
def build_embed(authorName, authorPicture, embedTitle, embedDesc, embedColor):
    emb = discord.Embed()
    emb.set_author(name=authorName, url="", icon_url=authorPicture)
    emb.title = embedTitle
...
async def on_message(message):
    if message.channel.id == int(sourceChannelID):
        ...
        embedDesc = message.created_at.strftime("%H:%M:%S - %d/%m/%Y")
        await send_message(build_embed(authorName, message.author.avatar_url, message.clean_content, embedDesc, message.author.color))
